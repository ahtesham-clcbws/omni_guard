...
use OmniGuard\Contracts\Wildcard;
use OmniGuard\Events\PermissionAttached;
use OmniGuard\Events\PermissionDetached;
use OmniGuard\Exceptions\GuardDoesNotMatch;
use OmniGuard\Exceptions\PermissionDoesNotExist;
use OmniGuard\Exceptions\WildcardPermissionInvalidArgument;
use OmniGuard\Exceptions\WildcardPermissionNotImplementsContract;
use OmniGuard\Guard;
...
trait HasPermissions
{
    ...
    public function getWildcardClass()
    {
        ...
        if (config('omniguard.enable_wildcard_permission')) {
            $this->wildcardClass = config('omniguard.wildcard_permission', WildcardPermission::class);
    ...
    public function permissions(): BelongsToMany
    {
        $relation = $this->morphToMany(
            config('omniguard.models.permission'),
            'model',
    ...
    public function scopePermission(Builder $query, $permissions, $without = false): Builder
    {
        ...
        return $query->where(fn (Builder $query) => $query
            ...
            ->whereIn(config('omniguard.table_names.permissions').".$permissionKey", \array_column($permissions, $permissionKey))
            )
            ->when(count($rolesWithPermissions), fn ($whenQuery) => $whenQuery
                ...
                ->whereIn(config('omniguard.table_names.roles').".$roleKey", \array_column($rolesWithPermissions, $roleKey))
                )
            )
        );
    }
